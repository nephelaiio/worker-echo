...
const server :Workerd.Config = (
  ...
  services = [ (name = "echo", worker = .echo) ],

  # Each configuration defines the sockets on which the server will listen.
  # Here, we create a single socket that will listen on localhost port ${WORKER_PORT}, and will
  # dispatch to the "main" service that we defined above.
  sockets = [ ( name = "http", address = "*:${WORKER_PORT}", http = (), service = "echo" ) ]
);
...
const echo :Workerd.Worker = (
  modules = [
    (name = "worker", esModule = embed "${WORKER_OUTPUT}")
  ],
  compatibilityDate = "${WORKER_COMPATIBILITY}",
);
